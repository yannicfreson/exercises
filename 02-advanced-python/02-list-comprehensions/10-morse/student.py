...
def to_morse(string):
    print('   '.join(' '.join(to_morse_table[char] for char in word) for word in string.upper().split(' ')))
    return '   '.join(' '.join(to_morse_table[char] for char in word) for word in string.upper().split(' '))

def from_morse(string):
    print(' '.join(''.join(from_morse_table[char] for char in word.split(' ')) for word in string.split('   ')))
